...
2. In the web service's **Environment** settings, add `DATABASE_URL` using
   the database's **Internal Database URL**.
3. Redeploy. The app creates the `students` table at startup, and dashboard
   records are then stored in Postgres.

Without `DATABASE_URL`, the app continues to use the local SQLite database.
